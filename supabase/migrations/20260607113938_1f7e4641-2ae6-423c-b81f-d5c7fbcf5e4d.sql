-- Update process_robot_bids to fix the logic where robots weren't bidding against each other
-- when there were no human bids.
...
BEGIN
    ...
    LOOP
        ...
        -- 1. Lógica principal de disputa contra HUMANO (TARGET_WINNER)
        ...
        -- Deve ocorrer se não houve lance humano ou se queremos manter o leilão vivo
        IF NOT v_should_bid AND COALESCE(v_settings.inner_dispute_enabled, false) AND v_auction.target_winner != 'user' THEN
            -- Se o tempo estiver baixo, os robôs disputam entre si
            -- Independente de quem deu o último lance (seja robô ou ninguém)
        ...
        IF v_should_bid THEN
            -- Escolhe um robô aleatório diferente do último licitante
