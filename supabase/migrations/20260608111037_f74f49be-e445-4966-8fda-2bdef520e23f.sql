...
BEGIN
    ...
    LOOP
        ...
        -- Se o tempo já acabou ou é irrelevante, não tenta dar lance
        IF v_time_remaining_ms <= 0 THEN
            CONTINUE;
        END IF;

        -- Usar o start_time real ou a criação como fallback
        -- Se o leilão foi reiniciado várias vezes, o start_time é o que importa
        ...
        -- Se já passou do tempo de parar em ambas as métricas, não dar lance
        ...
        IF v_time_remaining_ms < 3000 THEN
            -- Se humano é o último, ou se é robô vs robô e ainda está no prazo
            IF NOT v_last_bidder_is_bot THEN
                v_should_bid := true; -- Sempre cobre humano se estiver no prazo
            ELSIF COALESCE(v_settings.inner_dispute_enabled, false) AND v_is_within_dispute_limit THEN
                v_should_bid := (v_random_factor < 0.98); -- Quase sempre cobre outro robô nos últimos 3s
            END IF;
        ...
        ELSIF v_auction.target_winner = 'robot' OR (COALESCE(v_settings.inner_dispute_enabled, false) AND v_is_within_dispute_limit) THEN
            IF NOT v_last_bidder_is_bot THEN
                -- Se humano deu lance, cobre de forma natural entre 3s e 8s
                IF v_time_remaining_ms < 8000 THEN
                    v_should_bid := (v_random_factor < 0.8);
                END IF;
            ELSE
                -- Robô contra robô: Manter atividade esporádica para parecer natural
                IF v_time_remaining_ms < 10000 AND v_random_factor < 0.3 THEN
                    v_should_bid := true;
                END IF;
            END IF;
        ...
        ELSIF NOT v_last_bidder_is_bot AND v_is_within_stop_limit THEN
            IF v_time_remaining_ms < 5000 THEN
                v_should_bid := (v_random_factor < 0.9);
