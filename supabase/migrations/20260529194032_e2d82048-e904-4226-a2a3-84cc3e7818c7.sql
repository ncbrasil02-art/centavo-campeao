...
BEGIN
    ...
    LOOP
        ...
        -- Check dispute duration (Work Time)
        ...
        IF NOT v_should_bid THEN
            IF v_auction.target_winner = 'robot' THEN
                ...
                IF v_is_in_dispute_period THEN
                    -- During work time, simulate dispute
                    IF NOT v_last_bidder_is_bot THEN
                        -- Only bid when time is getting low or random chance to make it look "real"
                        IF v_time_remaining_ms < 10000 OR v_random_val < v_auction.bid_chance THEN
                            v_should_bid := true;
                        END IF;
                    END IF;
                ELSE
                    -- WORK TIME OVER: Robot only bids if it's NOT the winner, to "arrematar" (win)
            ...
            ELSIF v_auction.target_winner = 'user' THEN
                ...
                IF v_is_in_dispute_period THEN
                    ...
                    END IF;
                ELSE
                    -- Dispute over, robot stops to let user win
                    v_should_bid := false;
                END IF;
            ELSE
                ...
                IF v_is_in_dispute_period THEN
                    ...
                    END IF;
                ELSE
                    -- After work time, stop bidding
                    v_should_bid := false;
                END IF;
            END IF;
        END IF;

        -- Final safety check: if we should bid but the timer just reset, maybe wait a bit
