...
BEGIN
    ...
    LOOP
        ...
        -- Se o tempo já acabou ou é irrelevante, não tenta dar lance (evita erros de status)
        ...
        -- LÓGICA DE LANCES (AGRESSIVA PARA DISPUTAS)
        
        -- 1. RESGATE DE EMERGÊNCIA ABSOLUTO: Se o tempo está acabando (< 2s)
        IF v_time_remaining_ms < 2000 THEN
            IF NOT v_last_bidder_is_bot THEN
                -- Se humano é o último, robô configurado para ganhar OU em disputa DEVE cobrir
                v_should_bid := true;
            ELSIF COALESCE(v_settings.inner_dispute_enabled, false) AND v_is_within_dispute_limit THEN
                -- Disputa robô vs robô: Chance altíssima nos últimos segundos para manter o leilão vivo
                v_should_bid := (v_random_factor < 0.95); 
            END IF;
        
        -- 2. DISPUTA ATIVA (TEMPO > 2s)
        ELSIF v_auction.target_winner = 'robot' OR (COALESCE(v_settings.inner_dispute_enabled, false) AND v_is_within_dispute_limit) THEN
            IF NOT v_last_bidder_is_bot THEN
                -- Se humano deu lance, cobre de forma natural mas garantida abaixo de 10s
                IF v_time_remaining_ms < 5000 THEN
                    v_should_bid := true;
                ELSIF v_time_remaining_ms < 10000 AND v_random_factor < 0.7 THEN
                    v_should_bid := true;
                END IF;
            ELSE
                -- Robô contra robô: Manter atividade
                IF v_time_remaining_ms < 6000 AND v_random_factor < 0.4 THEN
                    v_should_bid := true;
                END IF;
            END IF;
        ...
        ELSIF NOT v_last_bidder_is_bot AND v_is_within_stop_limit THEN
            -- Garantia total nos últimos segundos se o alvo for robô ganhar ou random
            IF v_time_remaining_ms < 3000 THEN
                v_should_bid := (v_random_factor < 0.98);
            ELSIF v_time_remaining_ms < 7000 THEN
