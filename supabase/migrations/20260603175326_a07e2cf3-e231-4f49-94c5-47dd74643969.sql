-- 1. Corrigir e fortalecer process_robot_bids
...
BEGIN
    ...
    LOOP
        ...
        -- Lógica de disputa:
        -- Robô dá lance se o último não foi dele OU se o tempo estiver muito baixo (disputa interna)
        IF (NOT v_last_bidder_is_bot AND v_time_remaining_ms <= (v_auction.timer_duration * 1000 * 0.8)) 
           OR (v_time_remaining_ms <= 4000 AND random() < 0.3) THEN
            
            -- Seleciona robô
            SELECT id INTO v_robot_id FROM public.profiles WHERE is_bot = true ORDER BY random() LIMIT 1;
            
            IF v_robot_id IS NOT NULL THEN
                -- Chama a função place_bid correta (a que recebe 2 argumentos)
