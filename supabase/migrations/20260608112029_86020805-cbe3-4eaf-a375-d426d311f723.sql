...
BEGIN
    ...
    LOOP
        ...
        -- Se estiver acabando o tempo e estivermos no prazo de disputa/stop
        ...
        ELSIF v_auction.target_winner = 'robot' OR (COALESCE(v_settings.inner_dispute_enabled, false) AND v_is_within_dispute_limit) THEN
            IF NOT v_last_bidder_is_bot THEN
                -- Cobre humano entre 4s e 10s
                IF v_time_remaining_ms < 10000 THEN
                    v_should_bid := (v_random_factor < 0.7);
                END IF;
            ELSE
                -- Robô contra robô: Manter atividade esporádica
                IF v_time_remaining_ms < 12000 AND v_random_factor < 0.4 THEN
                    v_should_bid := true;
                END IF;
            END IF;
