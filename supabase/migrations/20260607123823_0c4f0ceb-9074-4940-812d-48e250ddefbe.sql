...
BEGIN
    ...
    LOOP
        ...
        -- LÓGICA ULTRA DETERMINÍSTICA (RESCUE MODE)
        
        -- Se o tempo expirou mas o leilão ainda está LIVE, o robô DEVE resgatar
        ...
        ELSIF v_auction.target_winner = 'robot' OR COALESCE(v_settings.inner_dispute_enabled, false) THEN
            ...
            IF NOT v_last_bidder_is_bot THEN
                v_should_bid := true;
            -- Se o último JÁ É robô, mas o tempo está abaixo de 15s (aumentado de 10s), outro robô entra para trocar lances
            ELSIF v_time_remaining_ms < 15000 THEN
                v_should_bid := true;
            END IF;
        -- Se for contra humano e tempo acabando (abaixo de 8s)
...
-- Reset the auction for GoPro and trigger a tick
