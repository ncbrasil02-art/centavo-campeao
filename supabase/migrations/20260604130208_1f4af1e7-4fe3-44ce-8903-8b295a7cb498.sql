-- Ajustando search_path para segurança (Good practice from linter)
...
-- Revogando acesso público a funções sensíveis
REVOKE EXECUTE ON FUNCTION public.process_robot_bids() FROM public;
REVOKE EXECUTE ON FUNCTION public.process_robot_bids() FROM anon;
GRANT EXECUTE ON FUNCTION public.process_robot_bids() TO service_role;
GRANT EXECUTE ON FUNCTION public.process_robot_bids() TO authenticated; -- Necessário para o Heartbeat no front-end

-- Garantir que a disputa interna seja respeitada e simular latência humana realista
...
BEGIN
    ...
    LOOP
        ...
        IF NOT v_last_bidder_is_bot THEN
            ...
            -- Se o tempo estiver abaixo de 1/3 do cronômetro original, ele se torna alerta.
            v_bid_threshold_ms := LEAST(v_auction.timer_duration * 1000 / 2, 4000); 
            
            IF v_time_remaining_ms <= 1300 THEN
                -- CRÍTICO: 1.3s para acabar. Se não der lance agora, o leilão pode fechar por lag de rede.
                -- Aumentamos a base de chance significativamente.
                IF v_random_factor < COALESCE(v_settings.bid_chance, 0.98) THEN
                    v_should_bid := true;
                END IF;
            ELSIF v_time_remaining_ms <= v_bid_threshold_ms THEN
                -- JANELA DE REAÇÃO: Chance progressiva.
                -- Quanto menor o bid_chance configurado, mais o robô "arrisca" deixar o tempo correr.
                IF v_random_factor < (COALESCE(v_settings.bid_chance, 0.8) * 0.75) THEN
                    v_should_bid := true;
                END IF;
            END IF;
        ...
        ELSIF COALESCE(v_settings.inner_dispute_enabled, false) AND v_time_remaining_ms <= 2800 THEN
            -- Chance de 45% de um robô cobrir outro para manter o leilão vivo
