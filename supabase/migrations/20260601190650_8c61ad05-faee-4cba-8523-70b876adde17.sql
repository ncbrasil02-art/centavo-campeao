        ...
        IF v_is_active_work_period THEN
            ...
            IF NOT v_last_bidder_is_bot THEN
                ...
                IF v_time_remaining_ms <= 3000 THEN
                    v_should_bid := true; -- Guaranteed bid in the last 3 seconds
                ELSIF v_time_remaining_ms <= 8000 THEN
                    IF v_random_val < 0.7 THEN v_should_bid := true; END IF; -- High probability
                ELSE
                    IF v_random_val < 0.15 THEN v_should_bid := true; END IF; -- Occasional bid to simulate life
                END IF;
            ELSE
                -- Robot is winning. 
            ...
            END IF;
        ELSE
            -- OUTSIDE WORK PERIOD (Work Period finished or set to 0)
